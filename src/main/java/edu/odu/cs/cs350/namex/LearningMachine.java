...
public class LearningMachine implements Serializable {
	...
	/**
	 * constructor for LearningMachine that 
	...
	/**
	 * returns String of shingle from tokens
	 * @param tokens
	 * @param k
	 * @return
	 */
	...
	/**
	 * returns shingles in HashSet<String> 
	 * @param tks
	 * @param k
	 * @param showOutput
	 * @return
	...
	public HashSet<String> getTrainingShingles(ArrayList<Token> tks, int k, boolean showOutput)
	{
		...
		// add k null tokens to the beginning of the arraylist and k null tokens
		// at the end
		for (int i = 0; i < k; i++) {
			tokens.add(0, new Token("null"));
			tokens.add(new Token("null"));
		...
		// build a shingle
		for (int i = 5; i < (tokens.size() - k); i++) {
			// System.out.println("LEXEME TO BE CLASSIFIED: " +
			...
			if (withinPER) {
				if (tokens.get(i).getLexical().equals("capitalized") || tokens.get(i).getLexical().equals("capLetter")
						|| tokens.get(i).getLexical().equals("allCaps")) {

					// set the classification of the token following a <PER> tag
					// to beginning
					if (tokens.get(i).getPER().equals("start")) {
						// System.out.println("<PER>start");
						// if after PER and before a comma, the name is in the
					...
					// if the token before previous token was an honorific then
					// a period, set to beginning
					else if (tokens.get(i - 2).isHonorific() == 1
							&& tokens.get(i - 1).getPartOfSpeech().equals("period")) {
						tokens.get(i).setName("beginning");
					}
					...
					// if the next token is a comma, and the previous was a
					// <PER> tag
					else if (tokens.get(i + 1).getPartOfSpeech().equals("comma")
							&& tokens.get(i - 1).getLexeme().equals("<PER>")) {
						tokens.get(i).setName("continuing");
					}

					// if the previous token was a comma and the one before that
					// was a continuing
					else if (tokens.get(i - 1).getPartOfSpeech().equals("comma")
							&& tokens.get(i - 2).getName().equals("continuing")) {
	...
	/**
	 * returns Attribute from a FastVector based on its name 
	 * @param name
	 * @param nominalVal
	 * @return
	 */
	...
	/**
	 * used to initialize a FastVector with lexical values
	 * @param nvl
	 */
	...
	/**
	 * used to initialize a FestVector with PartOfSpeech values
	 * @param nvp
	 */
	...
	/**
	 * Uses training data parameter to train the Learning Machine
	 * @param trainingData
	 * @throws Exception
	 */
	...
	/**
	 * builds classifier based on ARFF data
	 * constructs evaluation summary
	 * @throws Exception
	 */
	...
	/**
	 * returns distribution[] for Instance
	 * @param input
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * returns true if successfully printed evaluation summary
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * getter for evaluation summary string
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Exports training data to ARFF file
	 * @return
	...
	/**
	 * imports ARFF file from given path
	 * @param filePath
	 * @return
	 * @throws Exception
	 */
	...
	/**
	 * Imports ARFF file from given trainingdata HashSet
	 * 
	...
	/**
	 * Imports ARFF file from given training data string array
	 * 
	 * @param String[]
	 *            trainingData
	 */
	...
	/**
	 * getter for number of attributes being used
	 * 
	 * @return
	 */

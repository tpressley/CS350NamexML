...
public class TestExtractor {
	...
	{	
		...
		// add sample inputs
		inputs.add("<NER>Hello Gerard Silverio</NER><NER>Hello world! My name is Addy Alago</NER>");                  // first last
		inputs.add("<NER>Name Extraction -- Requirements Definition\nSteven J Zeil\nJan 20, 2016</NER>");             // first middle initial last
		inputs.add("<NER>Name Extraction -- Requirements Definition\nSteven J. Zeil\nJan 20, 2016</NER>");            // first middle initial period last
		inputs.add("<NER>Why is this test so cool Jack James Jameson?</NER>");                                        // first middle last
		...
		inputs.add("<NER>I went to George Washington University and studied Computer Science.</NER>");                // test kill word doesn't match
		inputs.add("<NER>You should go build a campfire Gary J. Jones, Jr.</NER>");                                   // last name suffix
		inputs.add("<NER>You should go build a campfire Gary J. Jones, III</NER>");                                   // last name suffix
		inputs.add("<NER>At school, James von Dutch was my Computer Science teacher.</NER>");                         // last name prefix
		inputs.add("<NER>I have never read any books written by Mark Twain or Jane Austen.</NER>");                   // DTIC authors
		
		// add pre-marked outputs to assert input text blocks
		outputs.add("<NER>Hello <PER>Gerard Silverio</PER></NER><NER>Hello world! My name is <PER>Addy Alago</PER></NER>");   // first last
		outputs.add("<NER>Name Extraction -- Requirements Definition\n<PER>Steven J Zeil</PER>\nJan 20, 2016</NER>");         // first middle initial last
		outputs.add("<NER>Name Extraction -- Requirements Definition\nSteven J. Zeil\nJan 20, 2016</NER>");                   // first middle initial period last
		outputs.add("<NER>Why is this test so cool <PER>Jack James Jameson</PER>?</NER>");                                    // first middle last
		...
		outputs.add("<NER>I went to George Washington University and studied Computer Science.</NER>");                       // test kill word doesn't match
		outputs.add("<NER>You should go build a campfire <PER>Gary J. Jones, Jr.</PER></NER>");                               // last name suffix
		outputs.add("<NER>You should go build a campfire <PER>Gary J. Jones, III</PER></NER>");                               // last name suffix
		outputs.add("<NER>At school, <PER>James von Dutch</PER> was my Computer Science teacher.</NER>");                     // last name prefix
		inputs.add("<NER>I have never read any books written by <PER>Mark Twain</PER> or <PER>Jane Austen</PER.</NER>");      // DTIC authors
	...
	public void testTokenizer()
	{
		...
		{
			assertEquals(tokens[i], assumedTokens[i]);
			System.out.println(i + ": " + (tokens[i].equals(assumedTokens[i])) + " (" + tokens[i] + "-" + assumedTokens[i] + ")");
		}
